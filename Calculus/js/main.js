...
const solve = (a,n) => {
	let an,answer;
	an = a * n;
	answer = `y = ${(a === 1 && n !== 0) ? `x` : (a === -1 && n !== 0) ? `-x` : a === 0 ? '0' : a !==(1||-1||0) && n !== 0 ? `${a}x`: a}<sup>${n === 1 ? `` : n === 0 ? ``: a === 0 ? `` :`${n}`}</sup><br/> 
						<sup>dy</sup>/<sub>dx</sub> = 
						${an === 0 ? `` : an}${(n-1 === 0) ? `` : (n === 0) ? `0` : `x<sup>${(n-1) === 1 ? `` : n - 1}</sup>`}`
						//${((an === 1) && (n - 1 === 0)) ? 1 : (an !== 1 && (n - 1 === 0)) ? an : an === 1 ? `x` : an === -1 ? `-x` : ``}${(n - 1) !== 0 ? `<sup>${n - 1}</sup>` : ``}`;
	return answer;
}
